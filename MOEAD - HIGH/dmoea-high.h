#ifndef __MOEAD_HIGH_H_
#define __MOEAD_HIGH_H_
...
public:
	...
	TMOEAD_HIGH(int n_H,int pops,int moead_nei);
	virtual ~TMOEAD_HIGH();
	...
	int cal_index_from_lambda(const vector<int> &V_obj);
	vector<int> gen_vicinities( vector<int> V_obj);
	...
	void partition_overlap_linear(int niche);
	...
	void operator=(const TMOEAD_HIGH &emo);
