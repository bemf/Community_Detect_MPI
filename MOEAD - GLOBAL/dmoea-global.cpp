...
TMOEAD_GLOBAL::TMOEAD_GLOBAL(int n_H,int pops,int moead_nei)
{
	start = clock();
	this->n_H = n_H;
	popsize = pops;
	...
	{
		pops_sub[i] += proc_nei * 2;
		pops_sub_start[i] -= proc_nei;
		if (pops_sub_start[i] < 0)
			pops_sub_start[i] = 0;
		if (pops_sub_start[i] + pops_sub[i] > popsize)
			pops_sub[i] = popsize - pops_sub_start[i];
	}
	pops_sub[numprocs - 1] += proc_nei;
	pops_sub_start[numprocs - 1] -= proc_nei;
	...
	recordUpdateofIndiv = new bool[popsize];
	//重置更新标志
	for(int i=0;i<popsize;i++)
		recordUpdateofIndiv[i] = false;
}

TMOEAD_GLOBAL::~TMOEAD_GLOBAL()
{
	delete pops_sub;
...
void TMOEAD_GLOBAL::init_population()
{
    for(int i=0; i<popsize; i++)
	{
...
void TMOEAD_GLOBAL::init_uniformweight()
{
    for(int i=0; i< popsize; i++)
	{
        TSOP sop;
...
//分解子问题、并计算观察向量
void TMOEAD_GLOBAL::gen_uniformweight(int start_obj_index, int max_value_left, vector<int> coordinate, int sd)
{
...
void TMOEAD_GLOBAL::init_neighborhood()
{
    double *x   = new double[popsize];
	int    *idx = new int[popsize];
	niche = niche <= popsize ? niche : popsize;
	for(int i=0; i < popsize; i++)
	{
...
void TMOEAD_GLOBAL::update_problem(CIndividual &indiv, int id)
{
...
bool TMOEAD_GLOBAL::update_idealpoint_from_outside()
{
	...
	return (isUpdateL || isUpdateR);
}

void TMOEAD_GLOBAL::sendIdealpoint(double *buf,bool &isSend,int target,MPI_Request* request)
{
	if(numprocs >1)
	{
		//多进程才使用，保证单进程也可以执行
...
bool TMOEAD_GLOBAL::update_idealpoint(CIndividual &ind)
{
	bool isUpdate = false;
	for(int n=0; n<nobj; n++)
	{
...
// update the reference point
bool TMOEAD_GLOBAL::update_idealpoint(const double* y_obj,int side)
{
	bool isUpdate = false;
	for (int n = 0; n<nobj; n++)
	{
...
void TMOEAD_GLOBAL::evolution()
{
	CIndividual child, child2;
	int endPos = pops_sub_start[mpi_rank] +  pops_sub[mpi_rank] -RRSize;
	for(int i= pops_sub_start[mpi_rank] + LLSize ; i< endPos; i++)
	{
		int   n  =  i;
...
void  TMOEAD_GLOBAL::execute(int mg, int irun, vector<double>& igd,vector<double>&hv, double &runtime)
{
	...
	for( gen=2; gen<=mg; gen++)
	{
		...
		if(numprocs > 1 && gen % comm_i == 0)
		{
			...
			if (!LLSize)
			{
				...
				int m = 1;
				int lcount = 0;
				for (int p = pops_sub_start[mpi_rank]; p< pops_sub_start[mpi_rank] + LLSize; p++)
				{
			...
			if (!RRSize)
			{
				...
				int m = 1;
				int rcount = 0;
				int endPos = pops_sub_start[mpi_rank] + pops_sub[mpi_rank];
				for (int p = endPos - RRSize; p < endPos; p++)
				{
					if (recordUpdateofIndiv[p])
					{
						TSOP &targetP = population[p];
						rsend_indiv[m++] = targetP.namda[0];
		...
		while (1)
		{
			...
			if (!isDone[L_RECV]) {
				...
				else
				{
					update_idealpoint_from_outside();
					if (LLSize)
						getAndUpdateIndivs(lrecv_indiv, s_indiv*LSize + 1, prevRank, &req_indiv[L_RECV]);
			...
			if (!isDone[R_RECV])
			{
				...
				else
				{
					update_idealpoint_from_outside();
					if (RRSize)
						getAndUpdateIndivs(rrecv_indiv, s_indiv*RSize + 1, nextRank, &req_indiv[R_RECV]);
				}
			}
...
void TMOEAD_GLOBAL::save_front(char saveFilename[1024])
{
    std::fstream fout;
...
void TMOEAD_GLOBAL::save_pos(char saveFilename[1024])
{
	std::fstream fout;
	fout.open(saveFilename,std::ios::out);
	for(int n=0; n<population.size(); n++)
...
void TMOEAD_GLOBAL::operator=(const TMOEAD_GLOBAL &emo)
{
    popsize        = emo.popsize;
...
void TMOEAD_GLOBAL::gather_pop_y()
{
...
void TMOEAD_GLOBAL::gather_populations()
{
...
void TMOEAD_GLOBAL::calc_qualities(double &igd,double& hv)
{//只有Root进程才使用
	//处理数据
	vector<vector<double> > pop_y;
	pop_y.reserve(popsize);
...
double TMOEAD_GLOBAL::calc_distance(vector<vector<double> >& ParetoFront)
{
	double distance = 0;
...
double TMOEAD_GLOBAL::calc_hypervolume(vector<vector<double> >& ParetoFront)
{
	wfg hv;
...
void TMOEAD_GLOBAL::getAndUpdateIndivs(double* b_recv_indiv,int datasize,int source,MPI_Request *request)
{
	//if(mpi_rank == 0)
	...
	{//接收到来自进程的个体更新
		...
		for (int p = 0; p < count; p++)
		{
			...
			for (int i = 0; i < nobj; i++)
				lambda.push_back(b_recv_indiv[m++]);

			//update idealpoint
			update_idealpoint(indiv);

			//根据权重向量计算对应种群下标
			int index = floor(lambda[0] * n_H + 0.5);
...
void TMOEAD_GLOBAL::sendIndiv(double* b_send_indiv,int index,int target,MPI_Request *request,bool &isBegin)
{
	//等待上一个理想点更新信息传送完成
	int flag = 1;
	if(isBegin)
...
void TMOEAD_GLOBAL::update_indiv_from_outside()
{
	if (numprocs > 1)
	{
		//左边更新
